...
/**
 * onedirectconf module
 *
 * @package mod_onedirectconf
 * @copyright  2020 Nopparut Saelim
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

require("../../config.php");
require_once("$CFG->dirroot/mod/onedirectconf/lib.php");
global $USER, $OUTPUT;

$protocol = ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off') || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
$CurPageURL = $protocol . $_SERVER['HTTP_HOST'] . '/mod/onedirectconf/';
...
if ($u) {  // Two ways to specify the module
    $conf = $DB->get_record('onedirectconf', array('id'=>$u), '*', MUST_EXIST);
    $cm = get_coursemodule_from_instance('onedirectconf', $conf->id, $conf->courseid, false, MUST_EXIST);

} else {
    $cm = get_coursemodule_from_id('onedirectconf', $id, 0, false, MUST_EXIST);
    $conf = $DB->get_record('onedirectconf', array('id'=>$cm->instance), '*', MUST_EXIST);
}
...
require_capability('mod/onedirectconf:view', $context);

// Completion and trigger events.
onedirectconf_view($conf, $course, $cm, $context);

$PAGE->set_url('/mod/onedirectconf/view.php', array('id' => $cm->id));

onedirectconf_print_header($conf, $cm, $course);
onedirectconf_print_heading($conf, $cm, $course);
onedirectconf_print_intro($conf, $cm, $course);

$allowedit  = has_capability('mod/onedirectconf:edit', $context);
$allowview  = has_capability('mod/onedirectconf:view', $context);
...
//    echo("<br><br><br>Teacher<br><br><br>");
    ...
    $out .= onedirectconf_teacher_form($table->data);
...
//    echo("<br><br><br>Student<br><br><br>");
    ...
    $out .= "<br>";
    $out .= onedirectconf_student_form($url_st);
    $out .= "<br><br>";
...
function onedirectconf_print_header($conf, $cm, $course) {
    global $PAGE, $OUTPUT;
 ...
 * @return void
 */
function onedirectconf_print_heading($conf, $cm, $course, $notused = false) {
    global $OUTPUT;
    echo $OUTPUT->heading(format_string($conf->name), 2);
}

/**
 * Print onedirectconf introduction.
 * @param object $conf
 * @param object $cm
 * @param object $course
 ...
 * @return void
 */
function onedirectconf_print_intro($conf, $cm, $course, $ignoresettings=false) {
    global $OUTPUT;

    $options = empty($conf->displayoptions) ? array() : unserialize($conf->displayoptions);
    if ($ignoresettings or !empty($options['printintro'])) {
        if (trim(strip_tags($conf->intro))) {
            echo $OUTPUT->box_start('mod_introbox', 'onedirectconfintro');
            echo format_module_intro('onedirectconf', $conf, $cm->id);
            echo $OUTPUT->box_end();
        }
    }
}
 ...
 * @return string
 */
function onedirectconf_teacher_form($data) {
    $form_data = '  ';
...
function onedirectconf_student_form($url) {
